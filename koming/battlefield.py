...
class Village:
    ...
    def add_troop(self, troop: _Troop):
        self.troops.append(troop)

    def add_defence(self, defence: _Defence):
...
class UIVillage(Village):
    ...
    def add_troop(self, troop: _Troop):
        super().add_troop(troop)
        ui_coord = self.scaled_to_ui_coord(troop.pos)
        tile_img = pygame.image.load(self._get_coc_obj_resource_path_(troop))
        self.screen.blit(tile_img, ui_coord)

    def add_defence(self, defence: _Defence):
        super().add_defence(defence)
        ui_coord = self.scaled_to_ui_coord(defence.pos)
